...
-- DOL (DATA DEFINITION LENGUAJE)
-- CREATE
-- ALTER
-- DROP

-- DML (DATA MANIPULACION LENGUAJE)
-- SELECT 
-- INSERT 
-- UPDATE
-- DELETE
...
-- crear tadistinbla a partir de la información de otra
...
-- tablas temporales
-- son tablas alojadas en memoria RAM y desaparecen al cortar la conexion con el servidor 
...
-- NOTA: el editor de MySQL workbench trae configurado por default una proteccion para evitar ejecutar 
-- comandos de borrado o actualización masiva. Hay que desativarla desde el menu Edición , preferencias 
-- editor de sql. Para mayor seguridad, reiniciar el MySQL Workbench.
-- otra manera desactivarla es por medio de codigo:
set sql_safe_updates=0; -- desactiva la protección 
set sql_safe_updates=1; -- activa la protección

delete from clientes;
-- elimito todos los registros de la tabla clientes
insert into clientes (nombre, apellido, dni, direccion) values ('Fiorella','Aranibal','6565532',null);
-- inserto un nuevo registro a la tabla clientes, el codigo será el último asginado +1

-- el comando truncante tambien vacía la tabla, pero borrando todos los registros simultaneamente,
-- no uno por uno como el el comando delete, por lo tanto es más veloz.
-- Este comando tambien reinicia la secuencia de los auto incrementables a cero.
truncate clientes;

-- actualizar valores de registro de una tabla 
-- comando update

insert into clientes select *from clientes_2;

update clientes set nombre='Mariana' where codigo=2;
-- actualizar la tabla clientes cambiando el nombre por 'Mariana'
-- a aquellos registros cuyos codigos sea igual a 2
