...
-- Las funciones de agrupamiento devuelve a un solo registro como resultado.
-- La función MAX trabaja con números, texto y fechas

-- MAX con números
select max(monto) from facturas;
-- trae el monto maximo de la tabla facturas
select max(monto) maximo from facturas; -- agrego un alias a la columna
select max(monto) maximo from facturas where letra='b'; -- trae el mayor monto de las facturas con letra 'b'

-- no debe haber un espacio entre el nombre de la funcion y el primer paréntesis
-- esto puede variar segun el motor de la Base de Datos 
...
-- trae el primer, teniendo en cuenta el orden alfabetico

-- MIN con fechas 
select min(fechas) primera_fecha_de_facturas from facturas;
...
-- devuelve el monto promedio ajustado a dos decimales
-- la funcios round() redondea. El primer parámetro es el número a redondear
-- y el segundo parámetro es la cantidad de decimales que queremos montrar.alter
...
-- trae la cantidad total de los registros de la tabla facturas 
...
-- GROUP BY
-- devuelve un registro por cada valor del campo agrupando
...
-- uso de having, se utiliza para filtrar por campo
select letra, 
count(*) total, 
sum(monto) suma
from facturas 
...
-- SUBQUERIES
select max(monto) from facturas; -- me devuelve 50.000 que es el monto maximo 
